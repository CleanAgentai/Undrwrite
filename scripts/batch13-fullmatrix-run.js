#!/usr/bin/env node
// BATCH 13 — full faithful 125-scenario re-run against staging (53f5773 Path-2 bundle).
// Superset of bulletproof-replay-batch.js: same select/cleanup/budget/continue-on-fail
// semantics, PLUS a compact raw-render capture per scenario so results-3.json carries the
// assertion-level data AND the Snapshot/Q5/Q8/Q10/Bug-3 render markers the BATCH-13 report
// (per-feature live-fire tallies) + BATCH-14 need. Sequential single pass. Self-cleans
// per scenario (cleanupRun: dealId + Phase-4 runTag-email sweep) + final sweep.
...
const OUT = path.join(__dirname, '..', 'test-fixtures', 'bulletproof', 'bulletproof-fullmatrix-results-3.json');
