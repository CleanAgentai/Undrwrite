...
(async () => {
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}. Loan amount: $260,000. Property ${property.full}.\n\n${broker.signoff}`,
  ...
  const correction = buildPostmarkPayload({
    ...
    textBody: `Updated loan amount: $295,000 — borrower decided to add the LOC consolidation per our conversation. All else unchanged.\n\n${broker.signoff}`,
  ...
  fs.writeFileSync(path.join(FIXTURE_DIR, 'events.json'),
    JSON.stringify(correctionSequence({ intake, correction, correctionDelayMs: 23.5 * 3600 * 1000 }), null, 2));
