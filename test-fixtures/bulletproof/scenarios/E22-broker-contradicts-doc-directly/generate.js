...
(async () => {
  ...
  // loan_app: $185k; broker email: $325k (same intake message)
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    borrowerName: borrower.fullName, propertyAddress: property.full,
    propertyValue: 650000, loanAmount: 185000,
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}. Loan amount $325,000 (please use this number; the loan application form attached still shows $185k from an earlier draft — borrower decided to up the ask after the form was signed).\n\n${broker.signoff}`,
  ...
  console.log(`[${SCENARIO_ID}] Generated 3 docs + 1-event sequence (broker direct contradiction at intake)`);
