...
(async () => {
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}. ${lender.name} first mortgage, payout statement attached.\n\nExit strategy: borrower intends to sell the property at end of term.\n\n${broker.signoff}`,
  ...
  const correction = buildPostmarkPayload({
    ...
    textBody: `Balance update — borrower made an unscheduled $2,500 principal payment yesterday (post-statement date). Current first mortgage balance is $222,500 not $225,000. Please use the updated figure for the underwriting math.\n\n${broker.signoff}`,
