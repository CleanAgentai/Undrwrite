...
const SCENARIO_ID = 'E06';

(async () => {
  fs.mkdirSync(DOCS_DIR, { recursive: true });
  const broker = BROKERS.jason_mercer, borrower = BORROWERS.david_okafor;
  const property = ADDRESSES.mississauga_winston;
  // 65% LTV purchase: $468k against $720k purchase, $252k down
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    borrowerName: borrower.fullName, propertyAddress: property.full,
    propertyValue: 720000, loanAmount: 468000,
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nPurchase for ${borrower.fullName}, ${property.full}. Purchase price $720,000, loan amount $468,000 (35% down — $252k). First mortgage.\n\n${broker.signoff}`,
