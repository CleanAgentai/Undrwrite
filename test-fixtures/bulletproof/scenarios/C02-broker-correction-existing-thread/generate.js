...
(async () => {
  ...
  // Routine correction — broker adds borrower phone number that was missing
  const correction = buildPostmarkPayload({
    from: broker.email, fromName: broker.name,
    subject: `Re: Refinance — ${borrower.fullName}`,
    textBody: `Hi Franco,\n\nForgot to include borrower phone earlier — ${borrower.fullName} can be reached at ${borrower.phone}. Best to leave voicemail; she'll call back same day.\n\n${broker.signoff}`,
