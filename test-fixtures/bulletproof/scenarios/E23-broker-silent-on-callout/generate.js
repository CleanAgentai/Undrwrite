...
(async () => {
  ...
  // Postal discrepancy across docs — Vienna would emit callout; broker doesn't reply
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    borrowerName: borrower.fullName, propertyAddress: `${street} T6R 2K8`,
  ...
  fs.writeFileSync(path.join(DOCS_DIR, 'mortgage_statement.pdf'), await synthMortgageStatement({
    borrowerName: borrower.fullName, propertyAddress: `${street} T6R 2K8`, lender: lender.name,
    balance: 225000, payoffAmount: 226500, interestRate: 5.45, validityDate: '2026-06-15',
  }));
  fs.writeFileSync(path.join(DOCS_DIR, 'appraisal.pdf'), await synthAppraisal({
    propertyAddress: `${street} T6R 2K9`, appraisedValue: 650000,
  }));
  const intake = buildPostmarkPayload({
    from: broker.email, fromName: broker.name,
    subject: `Refinance — ${borrower.fullName}`,
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName} at ${street}. Loan $280k.\n\n${broker.signoff}`,
  ...
  console.log(`[${SCENARIO_ID}] Generated 3 docs (postal discrepancy callout-trigger) + 1-event sequence (silent path)`);
