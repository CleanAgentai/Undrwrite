...
(async () => {
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}, $525k.\n\n${broker.signoff}`,
  ...
  const reaskQuestion = buildPostmarkPayload({
    from: broker.email, fromName: broker.name,
    subject: `Re: Refinance — ${borrower.fullName}`,
    textBody: `Hi Franco,\n\nQuick question on the prelim review — what's the typical rate-spread you're seeing this month for owner-occupied refis at this LTV band? Borrower asked. Also: what AML/PEP confirmation format works best (signed declaration, copy of ID, both)?\n\n${broker.signoff}`,
    messageId: `${SCENARIO_ID}-reask@bulletproof.synthetic`,
    date: '2026-05-17T13:00:00.000Z',
  });
  fs.writeFileSync(path.join(FIXTURE_DIR, 'events.json'),
    JSON.stringify(linearTurns([intake, reaskQuestion]), null, 2));
