...
const SCENARIO_ID = 'D04';

(async () => {
  fs.mkdirSync(DOCS_DIR, { recursive: true });
  const broker = BROKERS.jonathan_ferrara;
  const borrower = BORROWERS.marcus_webb;
  const street = '1142 Tory Road NW, Edmonton, AB';
  const lender = LENDERS.rbc;

  // Same street address, different postal codes across docs
  const loanAppAddress = `${street} T6R 2K8`;
  const apprAddress = `${street} T6R 2K9`;

  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    borrowerName: borrower.fullName, propertyAddress: loanAppAddress,
  ...
  console.log(`[${SCENARIO_ID}] Generated 3 docs + 1-event sequence (loan_app postal T6R 2K8; appraisal postal T6R 2K9)`);
