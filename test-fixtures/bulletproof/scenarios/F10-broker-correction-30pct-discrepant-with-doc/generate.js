...
(async () => {
  ...
  // loan_app $180k; broker correction $290k — 38% discrepancy from broker
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    borrowerName: borrower.fullName, propertyAddress: property.full,
    propertyValue: 720000, loanAmount: 180000,
  ...
  const correction = buildPostmarkPayload({
    ...
    textBody: `Correction — loan amount is $290,000, not the $180k on the loan app form (form has stale draft figure). Borrower added LOC consolidation after the form was signed.\n\n${broker.signoff}`,
