...
(async () => {
  ...
  const lender = LENDERS.scotia; // formal: "Scotiabank"

  // Loan_app uses shorthand; mortgage_statement uses formal name
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    ...
    existingFirstMortgageBalance: 380000, existingFirstMortgageLender: 'Scotia',
  }));
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName} at ${property.full}. Payout statement attached.\n\n${broker.signoff}`,
