...
(async () => {
  ...
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    ...
    propertyValue: 650000, loanAmount: 310000,
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}, $310,000 against $650k property.\n\n${broker.signoff}`,
  ...
  const correction1 = buildPostmarkPayload({
    ...
    textBody: `Updating loan amount: $345,000. Borrower added an additional $35k for renovation reserve. Property value unchanged.\n\n${broker.signoff}`,
    messageId: `${SCENARIO_ID}-correction-1@bulletproof.synthetic`,
    date: '2026-05-16T10:00:00.000Z',
  });

  const confirmation = buildPostmarkPayload({
    from: broker.email, fromName: broker.name,
    subject: `Re: Refinance — ${borrower.fullName}`,
    textBody: `Confirming $345,000 is final. Borrower signed off this morning. Ready for prelim review with the updated number.\n\n${broker.signoff}`,
    messageId: `${SCENARIO_ID}-confirmation@bulletproof.synthetic`,
    date: '2026-05-17T09:00:00.000Z',
  });

  fs.writeFileSync(path.join(FIXTURE_DIR, 'events.json'),
    JSON.stringify(multiCorrectionSequence([intake, correction1, confirmation]), null, 2));
