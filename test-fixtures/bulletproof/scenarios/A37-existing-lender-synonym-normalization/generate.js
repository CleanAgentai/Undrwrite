...
(async () => {
  ...
  // Loan_app uses "Royal Bank of Canada"; mortgage_statement uses "RBC Royal Bank"; broker uses "RBC" then "Royal Bank"
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    ...
    existingFirstMortgageBalance: 225000, existingFirstMortgageLender: 'Royal Bank of Canada',
  }));
  fs.writeFileSync(path.join(DOCS_DIR, 'mortgage_statement.pdf'), await synthMortgageStatement({
    borrowerName: borrower.fullName, propertyAddress: property.full, lender: 'RBC Royal Bank',
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}. RBC first mortgage, balance $225k per attached payout.\n\n${broker.signoff}`,
  ...
  const correction = buildPostmarkPayload({
    ...
    textBody: `Just to keep naming consistent across the file — please use "Royal Bank" in the prelim review (matches lender's preferred branding for the consumer-facing communications). Same entity as RBC / Royal Bank of Canada in the docs.\n\n${broker.signoff}`,
