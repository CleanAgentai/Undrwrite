...
(async () => {
  ...
  fs.writeFileSync(path.join(DOCS_DIR, 'loan_application.pdf'), await synthLoanApp({
    ...
    propertyValue: 650000, loanAmount: 120000,
    transactionType: 'Refinance', mortgagePosition: '1st mortgage',
    existingFirstMortgageBalance: 280000, existingFirstMortgageLender: lender.name,
  }));
  fs.writeFileSync(path.join(DOCS_DIR, 'mortgage_statement.pdf'), await synthMortgageStatement({
    ...
    balance: 280000, payoffAmount: 281500, interestRate: 5.45, validityDate: '2026-06-15',
  }));
  ...
  const intake = buildPostmarkPayload({
    ...
    textBody: `Hi Franco,\n\nRefi for ${borrower.fullName}. Property ${property.full}, $120k requested. Existing first mortgage ${lender.name} balance $280k.\n\n${broker.signoff}`,
  ...
  const correction = buildPostmarkPayload({
    ...
    textBody: `Quick clarification — this is actually a 2nd mortgage* (we're keeping the existing first with ${lender.name} in place; the $120k is being added behind). Mortgage position: 2nd mortgage*.\n\n${broker.signoff}`,
